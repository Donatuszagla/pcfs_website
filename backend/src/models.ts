...
const pageSchema = new Schema({ title: { type: String, required: true }, slug: { type: String, required: true, unique: true }, description: String, body: String, image: String, sections: [{ type: { type: String }, enabled: Boolean, order: Number, content: Schema.Types.Mixed }], status: { type: String, default: "DRAFT", index: true }, seo: seoSchema, publishedAt: Date }, baseOptions);
